...
export class Trigger {
    ...
    collideRect(rect: number[]) {
        if (this.position[0] < rect[0]+rect[2] && this.position[0]+this.size[0] > rect[0] && this.position[1] < rect[1]+rect[3] && this.position[1]+this.size[1] > rect[1]) {
            switch(this.type) {
                case('toggle'):
                    if (!this.colliding) this.triggered = !this.triggered
                    break;
